...
def build_system_prompt(
    ...
    stable = (
        ...
        f"- 직전 사용자 답변에서 새로 확정된 정보는 다음 턴으로 미루지 말고, 이번 응답의 insights에 즉시 포함합니다(한 턴 지연 금지).\n"
        f"- 당신의 역할은 '질문으로 정보를 수집'하는 것뿐입니다. 인터뷰 도중 문서 작성·생성·"
        f"완성을 언급하거나 약속하지 않습니다(예: \"문서를 작성해드릴게요\" 금지). 문서는 인터뷰가 "
        f"끝난 뒤 별도 단계에서 자동으로 만들어집니다.\n"
        f"- '현재 단계 지시사항'의 주제에만 집중합니다. 다음 단계로 건너뛰거나, 지금까지의 수집 내용을 "
        f"미리 요약하지 않습니다. 전체 요약은 마지막 '마무리 확인' 단계에서 단 한 번만 합니다.\n\n"
        f"프로젝트: {project_name}\n"
    )
