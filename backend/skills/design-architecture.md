...
9. **AI 모델 미확정 처리**: 사용자가 사용할 AI 모델/벤더를 확정하지 않았으면(인터뷰 인사이트에 "추후 결정/미정", 또는 입력에 "AI 모델 미확정" 표기), AI 관련 컴포넌트의 technology에 특정 모델/벤더(GPT-4, Claude 등)를 단정하지 말고 `"LLM API (모델 추후 결정)"`처럼 미확정 상태로 표기한다. 사용자가 예시로 든 모델명("GPT-4 같은")을 결정으로 간주하지 않는다. 특정 벤더로 편향되지 않는다.
