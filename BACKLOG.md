...
**상태**: 🆕 대기 (기록만, 2026-07-14). 수익화 논의에서 도출 — "제일 비싼 인터뷰를 무료로 주면 뭘로 수익화?"의 **핵심 해법**. **비용을 가치와 정렬**: 공짜·고볼륨(인터뷰)은 싼 모델, 돈 받는·저볼륨(평가/최종문서)에만 비싼 모델.
**발견일**: 2026-07-14
...
### 추천 순서
1. 상위 티어(평가/최종문서) 먼저 격상 — 돈값·품질↑, [[BL-018]] F1과 세트. (리스크 낮음, 즉시 가치)
2. 경량 티어(인터뷰) 다운 — **품질 실측 게이트 통과 시에만.** [[BL-003]] 캐싱과 병행해 비용 최대 절감.
