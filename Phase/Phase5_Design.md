...
| 2026-07-21 | BL-021 1단계 구현: 설계·마감 세션 공통 인가 헬퍼 `require_owned_session` 추가. 세션→프로젝트→소유자를 검증하고, 삭제 프로젝트를 제외하며, 없는 세션과 접근 불가 세션에 동일한 404를 반환한다. 헬퍼 테스트 6개 통과. 설계 API 7개 연결은 남아 있어 IDOR는 아직 차단되지 않았다. |
| 2026-07-21 | BL-021 2단계 구현: 세션 ID 기반 설계 조회·수정 API 7개 모두에 `require_owned_session`을 연결했다. 조회는 헬퍼가 반환한 인가된 세션을 사용하고, 수정은 데이터 변경 전에 소유권을 검사한다. 설계·마감 API 8개를 함께 검증하는 엔드포인트 매트릭스 32건에서 소유자 200, 타 사용자·삭제 프로젝트·없는 세션 404, 거부 후 원본 불변을 확인했고 백엔드 전체 131개 테스트가 통과했다. `_get_or_create_session` 방어 강화는 남아 있다. |
| 2026-07-21 | BL-021 3단계 구현: `_get_or_create_session`이 공통 DB 클라이언트와 사용자 ID를 받아 세션 조회·생성 전에 `_require_design_access`를 실행하고 `(project, session)`을 반환하도록 변경해 생성 호출부의 중복 프로젝트 조회를 제거했다. 설계·마감 직접 헬퍼 테스트 10건으로 인가 우선 순서, 소유 세션 재사용, 신규 세션 1회 생성을 확인했고 백엔드 전체 141개 테스트가 통과했다. 코드 구현은 완료됐으며 프로덕션 전 보안 회귀 검증은 남아 있다. |
| 2026-07-21 | BL-021 4단계 완료: 실제 Supabase에 임시 Auth 사용자 2명을 만들고 각 사용자의 실제 로그인 JWT로 로컬 백엔드를 검증했다. 소유자 요청 8개는 모두 200, 타 사용자 교차 요청 8개와 프로젝트 soft delete 후 소유자 요청 8개는 모두 동일한 404였으며, 거부된 수정으로 데이터가 바뀌지 않았다. 이 과정에서 `maybe_single()` 무결과가 `None`일 수 있음을 확인해 `require_owned_session`이 응답 객체와 `None`을 모두 처리하도록 보완하고 회귀 테스트 2개를 추가했다. 백엔드 전체 143개 테스트가 통과했고 임시 Auth·공개 데이터는 모두 정리했다. |

---

## 수정 예정 — UI 테스트 이슈 (2026-06-02)

> 설계 플로우 실제 사용 테스트 중 발견. 우선순위별 정리.

### 즉시 수정 완료

| ID | 내용 | 수정 파일 | 상태 |
|---|---|---|---|
| P5-06 | "AI(Claude)" → "AI"로 표기 변경 | `DesignPage.tsx:51`, `AiWorkflowStep.tsx:122,156` | ✅ 완료 |
| P5-07 | AI 흐름 헬퍼 패널 비용 알림 섹션 삭제 | `AiHelperPanel.tsx:38-51` | ✅ 완료 |

### 1순위: 시스템 구조 추천 조합 단일화 (P5-01)

> 기존 이슈 #1, #2, #3 통합 해결

**변경 방향**: 추천 조합 2개 선택 UI 제거 → 추천 1개만 표시 후 바로 생성

| 파일 | 수정 내용 |
|------|-----------|
| `ArchitectureStep.tsx` (생성 전) | 2개 템플릿 그리드 → 추천 1개 카드로 축소. `selectedTemplate` state 제거. 바로 "AI로 아키텍처 설계하기" 버튼 노출 |
| `ArchitectureStep.tsx` (생성 후) | 134~151행 "추천 조합" 템플릿 선택 섹션 전체 제거. "선택한 조합 미리보기" 태그 제거 |
| `design.py` (백엔드) | `_TEMPLATE_PROMPT`에서 조합 2개 → 1개로 축소, 또는 템플릿 API 단순화 |

**해결되는 문제**:
- 조합 전환해도 다이어그램 안 바뀌는 버그 → 전환 자체가 없어지므로 해소
- "각 부품을 왜 골랐나요?" 안 바뀌는 버그 → 동일하게 해소
- 미리보기에 AI 아키텍처 누락 가능성 → 프롬프트 보강으로 함께 해결

### 2순위: AI 워크플로우 JSON 구조화 (P5-02 + P5-03)

> 기존 이슈 #11, #12 통합 해결

**현상**: AI 입출력 흐름이 기본값("사용자 요청 데이터", "AI 처리 결과")만 표시 + 폴백 전략 표시 깨짐
**원인**: 백엔드가 ai_workflow를 마크다운 원문으로 저장 → 프론트가 regex 파싱 → 실패 시 하드코딩 기본값

| 파일 | 수정 내용 |
|------|-----------|
| `design-ai-workflow` 스킬 프롬프트 | JSON 응답 형식 명시 (inputs, outputs, model, task, fallbacks 필드) |
| `design.py:626-692` | ai-workflow 생성 시 마크다운 원문 대신 JSON 파싱 후 구조화 데이터로 저장 |
| `AiWorkflowStep.tsx:14-79` | regex `parseAiWorkflow` 제거 → 구조화된 JSON 직접 렌더링 |
| `types.ts` | `ai_workflow` 타입을 `string`에서 구조화된 객체로 변경 |
| `schemas/design.py` | `ai_workflow` 스키마를 문자열에서 객체 모델로 변경 |

### 3순위: 아키텍처 다이어그램 AI 레이어 누락 (P5-04)

| 파일 | 수정 내용 |
|------|-----------|
| `design-architecture` 스킬 프롬프트 | AI/ML 유형 프로젝트일 때 AI 관련 컴포넌트를 components에 반드시 포함하라는 지시 추가 |
