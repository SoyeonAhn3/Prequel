...
| 2 | 인터뷰 진행률 시각화 — 스텝 프로그레스바 표시 | MVP-1 |
...
| 행위 | 차감 | 시점 |
|---|---|---|
| 킥오프 완료 (인터뷰 + 평가 + 마무리 + 문서 생성) | 1회 | 최종 킥오프 문서 생성 완료 시 |
| 설계 진행 (아키텍처/데이터 모델/AI 워크플로우) | 1회 | 설계 단계 시작 시 |

> 예시: 킥오프만 → 1회 차감 / 킥오프 + 설계 → 2회 차감
...
| 과금 구조 | 횟수제 — 킥오프 완료 시 1회, 설계 진행 시 1회 추가 차감. 수익 > 원가 유지 (MVP-2에서 결제 연동) |
...
| 2 | 사용자 횟수 소진 (킥오프/설계) | 추가 사용 차단 | 유료 전환 유도 안내 (MVP-2) |
...
| FR-010 | 인터뷰 진행률 시각화 | 스텝 프로그레스바를 인터뷰 중 실시간 표시한다 | 필수 | 제안#2 |
...
| FR-012 | 횟수 기반 사용 제한 | 횟수제 과금: ①킥오프 완료(인터뷰+평가+마무리+문서 생성) 시 1회 차감, ②설계 진행 시 1회 추가 차감. 잔여 횟수 0이면 차단 후 유료 전환 안내 표시 | 필수 | 섹션 1 과금 + 갭Q3 |
...
7. 기획 인터뷰 시작 — 채팅 UI에 질문이 순차 표시 (프로그레스바 표시)
...
- **사용 횟수 초과**: 잔여 횟수 0인 상태에서 킥오프 문서 생성 또는 설계 시작 시 → 차단 안내 + "유료 플랜 안내" 표시 (MVP-2에서 결제 연동)
...
| 비즈니스 | 횟수 소진 시 결제 없이 차단 — 킥오프 완료 +1회, 설계 진행 +1회 (MVP-1에 결제 미포함) | 갭 인터뷰 Q3 |
...
| credits_used | INTEGER | NOT NULL, DEFAULT 0 | 누적 사용 횟수 (킥오프 완료 +1, 설계 진행 +1) |
...
| ID | 규칙 | 대상 | 검증 시점 |
|---|---|---|---|
| IR-001 | 플랜별 잔여 횟수 검증: credits_used < 플랜 한도 (Free=2, Basic=10, Pro=30) | users.credits_used | 킥오프 문서 생성 시 + 설계 시작 시 (API) |
