...
export default function ChatCenter({
  ...
  }, [messages, currentQuestion, sending])

  useEffect(() => {
    if (!sending) setPendingMessage('')
  }, [sending])
  ...
  const handleQuickAction = (actionText: string) => {
    if (sending) return
    setPendingMessage(actionText)
    onSend(actionText)
  }

  const visibleMessages = showAllHistory ? messages : messages.slice(-4)
      ...
      <div className="px-7 py-2.5 border-b border-border bg-surface flex items-center gap-2.5 shrink-0">
        {currentQuestion ? (
          <>
            ...
            <span className="text-[12.5px] text-text font-semibold">
              {currentQuestion.stepTitle}
            </span>
            <span className="text-xs text-text-subtle">&rsaquo;</span>
            <span className="text-xs text-text-muted">
              {currentQuestion.questionNumber}번째 질문
            </span>
          </>
        ) : (
          <span className="text-xs text-text-muted">인터뷰 진행 중</span>
        )}
        ...
        <button
          onClick={onPause}
          className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs text-text-muted bg-transparent border border-border rounded-lg hover:bg-surface-alt transition-colors cursor-pointer"
        >
          <Pause size={12} />
          일시정지
        </button>
        ...
        <div className="max-w-[760px] mx-auto">
          ...
          {messages.length > 0 && (
            <div className="opacity-55 mb-1">
              <div
                className="text-[10.5px] text-text-subtle font-mono mb-2.5 flex items-center gap-2"
                style={{ letterSpacing: 0.4 }}
              >
                <span>이전 대화</span>
                ...
                {messages.length > 4 && (
                  ...
                  >
                    {showAllHistory ? '접기' : `모두 보기 (${messages.length})`}
                  </span>
                )}
              </div>
          ...
          {currentQuestion && (
            <div className="mt-3.5 mb-[18px]">
              <div
                className="text-[10.5px] text-accent font-bold font-mono mb-2.5 flex items-center gap-2"
              ...
              <div
                ...
                style={{
                  ...
                  boxShadow:
                    '0 1px 0 rgba(0,0,0,.02), 0 10px 28px -14px color-mix(in srgb, var(--color-accent) 40%, transparent)',
                }}
              >
                {/* Topic strip */}
                {currentQuestion.topics.length > 0 && (
                  ...
                  >
                    <span
                      className="text-[10.5px] font-mono text-accent-deep font-bold"
                      style={{ letterSpacing: 0.4 }}
                    >
                      주제
                    </span>
                    ...
                    {currentQuestion.importance && (
                      <span className="text-[11px] text-accent-deep opacity-70">
                        {currentQuestion.importance}
                      </span>
                    )}
                  </div>
                ...
                <div className="px-6 py-[22px] flex gap-3.5 items-start">
                  ...
                  <div className="flex-1">
                          ...
                          <div className="mt-2 pt-2 border-t border-dashed border-border text-xs text-text-muted leading-relaxed">
                            {currentQuestion.exampleAnswers.map((ex, i) => (
                              <div
                                key={i}
                                className={
                                  i < currentQuestion.exampleAnswers.length - 1 ? 'mb-1.5' : ''
                                }
                              >
                                &bull; <span className="text-text">{ex.label}</span>: {ex.text}
                              </div>
                            ))}
                          </div>
                        )}
                    ...
                    <div className="flex items-center gap-3.5 mt-3 text-[11px] text-text-subtle">
                      ...
                      <span className="flex items-center gap-1.5">
                        <FileText size={11} />
                        답변 &rarr; {currentQuestion.insightCount}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex gap-2 px-6 py-3.5 border-t border-border">
                  <button
                    onClick={() => handleQuickAction('이 질문에 대한 답변을 추천해주세요')}
                    disabled={sending}
                    className="inline-flex items-center gap-1.5 px-3.5 py-2 text-[13px] font-semibold bg-accent text-white rounded-full cursor-pointer border-none disabled:opacity-50 disabled:cursor-not-allowed"
                    style={{
                      boxShadow:
                        '0 2px 8px -2px color-mix(in srgb, var(--color-accent) 50%, transparent)',
                    }}
                  >
                    <Sun size={14} />
                    AI 추천받기
                  ...
                  <button
                    onClick={() => handleQuickAction('이 질문은 건너뛰겠습니다')}
                    disabled={sending}
                    className="inline-flex items-center gap-1.5 px-3.5 py-2 text-[13px] font-medium bg-surface text-text border border-border-strong rounded-full cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <ChevronsRight size={13} />
                    건너뛰기
                  </button>
                  <button
                    onClick={() => handleQuickAction('이 질문을 다른 방식으로 다시 해주세요')}
                    disabled={sending}
                    className="inline-flex items-center gap-1.5 px-3.5 py-2 text-[13px] font-medium bg-surface text-text border border-border-strong rounded-full cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <RotateCcw size={13} />
                  ...
                  <div className="flex-1" />
                  <button
                    onClick={() => handleQuickAction('다음 단계로 넘어가겠습니다')}
                    disabled={sending}
                    className="inline-flex items-center gap-1.5 px-3.5 py-2 text-[13px] font-semibold bg-accent-soft text-accent border border-accent rounded-full cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed hover:bg-accent hover:text-white transition-colors"
                  >
                    <ChevronsRight size={13} />
                    다음 단계로
                  </button>
          ...
          {/* Pending user message + AI thinking (shown while sending) */}
          {sending && pendingMessage && (
            <div className="mt-3.5 mb-[18px]">
              <div className="flex gap-2.5 items-start flex-row-reverse mb-3">
                ...
                <div className="max-w-[480px]">
                  <div className="rounded-[10px] px-3.5 py-2.5 text-[13px] leading-relaxed bg-accent text-white">
                    {pendingMessage}
                  </div>
                </div>
              </div>
              <div className="flex gap-2.5 items-start">
                <AiMark size={26} />
                <div className="bg-surface rounded-[10px] px-4 py-3 border border-border">
      ...
      <div className="px-7 pb-[18px] bg-bg shrink-0">
        <div className="max-w-[760px] mx-auto">
          ...
          <div
            ...
            style={{
              border: `1.5px solid ${sending ? 'var(--color-border)' : 'var(--color-accent)'}`,
              boxShadow: sending
                ? 'none'
                : '0 0 0 4px var(--color-accent-soft), 0 2px 12px -4px rgba(28,31,38,.08)',
            }}
          >
            <textarea
              value={input}
              ...
              disabled={sending}
              placeholder={
                sending ? 'AI가 응답을 준비하고 있습니다...' : '답변을 입력하세요...'
              }
              className="w-full text-sm text-text leading-relaxed font-sans resize-none bg-transparent outline-none disabled:text-text-muted"
            />
            <div className="flex items-center gap-3 mt-1 pt-2 border-t border-border">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 bg-accent text-white text-xs font-semibold rounded-lg border-none cursor-pointer hover:bg-accent-deep transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
              >
                <Send size={12} />
                {sending ? '전송 중...' : '전송'}
              </button>
          ...
          <div className="flex items-center gap-[18px] mt-2.5 px-1 text-[11px] text-text-subtle">
            <span className="flex items-center gap-1.5">
              <Clock size={11} />
              진행 시간{' '}
              <span className="text-text font-semibold font-mono">{stats.elapsedTime}</span>
            </span>
            <span className="w-[3px] h-[3px] bg-border-strong rounded-full" />
            <span>
              답변{' '}
              <span className="text-text font-semibold font-mono">{stats.answerCount}개</span>
            </span>
            <span className="w-[3px] h-[3px] bg-border-strong rounded-full" />
            <span>
              평균 답변 시간{' '}
              <span className="text-text font-semibold font-mono">{stats.avgAnswerTime}</span>
            </span>
            <div className="flex-1" />
